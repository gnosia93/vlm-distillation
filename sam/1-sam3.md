...
![](https://github.com/gnosia93/vlm-distillation/blob/main/images/sam-3-ft.png)
...
(1) "잘 판별 못한다"는 결론이 애초에 어떻게 나온 거냐, 그리고 (2) 그게 인코더 탓인지 어떻게 가려내냐. 순서대로 가야 한ㄷ. (2)를 하기 전에 (1)이 먼저이다.
...
실패 사례 프레임을 모으고, 정답 라벨을 붙여 "무엇을 얼마나 못 맞히는지"를 숫자로 만드는 것이다. 

#### 2. 병목이 어디인지 가려내는 진단 테스트 ####
"못 한다"를 확인했으면, 어느 부품 탓인지를 아래 테스트로 좁힌다.

#### ① 에러 유형 분류 (제일 먼저) ####
실패가 어떤 종류인지 본다. 이것만으로도 범인이 좁혀진다.
