...
1.	`메모리 병목 (HBM 쓰기)`: 기본 어텐션 연산은 중간 계산 결과물인 어텐션 행렬(‭$N \times N$‬‭‬)을 GPU의 느린 메인 메모리(HBM)에 저장했다가 다시 읽어와야 합니다. 시퀀스가 길어지면 이 행렬이 너무 커져 HBM 용량을 초과하거나, 데이터를 읽고 쓰는 속도가 연산 속도를 따라가지 못하는 병목이 발생합니다.
...
* 효과: ‭$N \times N$‬‭‬ 크기의 거대한 중간 어텐션 행렬을 HBM에 저장할 필요를 완전히 없애, 메모리 사용량을 ‭$N^2$‬에서 ‭$N$‬ 수준으로 줄입니다.
